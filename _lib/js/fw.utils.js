...
function displayDebugInfo(debug){
	...
	var $debug = $(debug),
		...
		$imgs = $('img'),
		on = '',
		updateImgSrc = function(skinned){
			
			$.each($imgs, function(idx, ele){
				var $img = $(ele),
					src = $img.attr('src'),
					custom = src.indexOf('custom');
				if ( skinned ) {
					if (custom < 0) src = src.replace( '_lib/img/', '_lib/img/custom/' );
				} else {
					src = src.replace( '_lib/img/custom/', '_lib/img/' );
				}
				$img.attr('src', src);
			});
			
		},
		showCustomSkin = function(){
			$body.addClass('skinned');
			updateImgSrc(true);
		},
		hideCustomSkin = function(){
			$body.removeClass('skinned');
			updateImgSrc(false);
		},
		toggle = function() {
			if ( on == 'true' ) {
